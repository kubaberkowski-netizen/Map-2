...
replaceOnce(
  ...
  "plRef=(0,r.useRef)(null),prz=(0,r.useRef)(!1),pacc=(0,r.useRef)(0),pps=(0,r.useRef)(0),awid=(0,r.useRef)(null),awtimer=(0,r.useRef)(null),awbreak=(0,r.useRef)(!1),awnative=(0,r.useRef)(!1),awlistener=(0,r.useRef)(null),awctx=(0,r.useRef)(\"\"),awfin=(0,r.useRef)(!1),awroute=(0,r.useRef)(null),awconsume=(0,r.useRef)(null),awelapsed=(0,r.useRef)(null),awwatch=(0,r.useRef)({listener:null,timer:null,last:\"\",pending:null,queue:Promise.resolve()}),awwatchcmd=(0,r.useRef)(null),[Eo,Sc]"
);
...
replaceOnce(
  ...
  '(0,r.useEffect)(()=>{awroute.current=runw,awconsume.current=awConsume,awwatchcmd.current=awWatchCommand,awnative.current&&awSyncContext(),awWatchPublish(!1)}),(0,r.useEffect)(()=>{let _live=!0,_wp=awWatchPlugin();if(_wp&&_wp.addListener)Promise.resolve(_wp.addListener("watchCommand",function(_ev){let _fn=awwatchcmd.current,_q=awwatch.current.queue||Promise.resolve();awwatch.current.queue=_q.then(function(){return _fn&&_fn(_ev)},function(){return _fn&&_fn(_ev)})})).then(function(_h){if(_live)awwatch.current.listener=_h;else try{_h&&_h.remove&&_h.remove()}catch(_){}}).catch(function(){});return()=>{_live=!1;let _w=awwatch.current,_h=_w.listener;_w.listener=null,_w.timer&&(clearTimeout(_w.timer),_w.timer=null);try{_h&&_h.remove&&_h.remove()}catch(_){}}},[])'
);
...
replaceOnce(
  ...
  'AWK="flaneur-active-walk-v1",awPlugin=()=>{try{',
  `AWK="flaneur-active-walk-v1",awWatchPlugin=()=>{try{return window.Capacitor&&window.Capacitor.Plugins&&window.Capacitor.Plugins.WatchSessionBridge||null}catch(_){return null}},awWatchTarget=_x=>({id:_x.id,name:_x.name||"Nearby place",latitude:+_x.latitude,longitude:+_x.longitude,category:_x.category||null,isCheckedIn:!!(_x.isCheckedIn||_x.isCompleted||O.has(_x.id)||P.has(_x.id))}),awWatchSnapshot=_state=>{try{let _m=awid.current;if(!_m||(!_state&&!is.current))return null;let _items=awItems(),_route=_items.routeStops.map(awWatchTarget),_near=_items.radarCandidates.map(awWatchTarget),_started=+_m.startedAt||+wg||Date.now(),_anchor=prz.current&&pps.current?+pps.current:Date.now(),_elapsed=Math.max(0,Math.floor((_anchor-_started-Math.max(0,+pacc.current||0))/1e3)),_points=ss.current||[],_last=_points[_points.length-1],_pos=Ia&&isFinite(+Ia.lat)&&isFinite(+Ia.lng)?Ia:_last,_location=null;if(_pos&&isFinite(+_pos.lat)&&isFinite(+_pos.lng)){_location={latitude:+_pos.lat,longitude:+_pos.lng,timestamp:+(_last&&_last.t||dl.current||Date.now())};isFinite(+Ki)&&(_location.accuracy=+Ki)}let _next=_items.routeStops.find(function(_x){return!O.has(_x.id)&&!P.has(_x.id)}),_snapshot={schema:1,sessionId:_m.id,state:_state||(prz.current?"paused":"recording"),startedAt:_started,elapsedSeconds:_elapsed,distanceMeters:Math.max(0,Math.round((+ds.current||0)*1e3)),routeStops:_route,nearbyTargets:_near,nextStopId:_next&&_next.id||null,radarRangeMeters:+Ua||300,updatedAt:Date.now()};return _location&&(_snapshot.currentLocation=_location),_snapshot}catch(_){return null}},awWatchSignature=_s=>{try{let _p=_s.currentLocation||{};return JSON.stringify([_s.sessionId,_s.state,Math.round(+_s.distanceMeters||0),+_p.latitude||0,+_p.longitude||0,+_p.timestamp||0,_s.nextStopId,+_s.radarRangeMeters||300,(_s.routeStops||[]).map(function(_x){return[_x.id,_x.isCheckedIn]}),(_s.nearbyTargets||[]).map(function(_x){return _x.id})])}catch(_){return""}},awWatchPublish=(_force,_state)=>{try{let _wp=awWatchPlugin(),_snapshot=awWatchSnapshot(_state);if(!_wp||!_wp.publish||!_snapshot)return Promise.resolve(!1);let _w=awwatch.current;_w.pending=_snapshot;let _send=function(){_w.timer=null;let _next=_w.pending;_w.pending=null;if(!_next)return Promise.resolve(!0);let _sig=awWatchSignature(_next);if(_sig&&_sig===_w.last)return Promise.resolve(!0);_w.last=_sig;return Promise.resolve(_wp.publish({snapshot:_next})).then(function(){return!0}).catch(function(){_w.last="";return!1})};if(_force){_w.timer&&(clearTimeout(_w.timer),_w.timer=null);return _send()}if(!_w.timer)_w.timer=setTimeout(_send,800);return Promise.resolve(!0)}catch(_){return Promise.resolve(!1)}},awWatchClear=()=>{try{let _w=awwatch.current;_w.timer&&(clearTimeout(_w.timer),_w.timer=null),_w.last="",_w.pending=null;let _wp=awWatchPlugin();return _wp&&_wp.clear?Promise.resolve(_wp.clear({})).catch(function(){return!1}):Promise.resolve(!1)}catch(_){return Promise.resolve(!1)}},awWatchAck=(_id,_success,_error,_snapshot)=>{try{let _wp=awWatchPlugin();if(!_wp||!_wp.acknowledge)return Promise.resolve(!1);let _payload={commandId:_id,success:!!_success};return _error&&(_payload.error=String(_error)),_snapshot&&(_payload.snapshot=_snapshot),Promise.resolve(_wp.acknowledge(_payload)).then(function(){return!0}).catch(function(){return!1})}catch(_){return Promise.resolve(!1)}},awWatchCommand=async _ev=>{let _id=_ev&&_ev.commandId,_command=_ev&&_ev.command,_ack=async(_ok,_error,_snapshot)=>{_id&&await awWatchAck(_id,_ok,_error,_snapshot)};try{if(!_id)return;if(+_ev.schema!==1){await _ack(!1,"Unsupported command");return}let _m=awid.current;if(!_m||_ev.sessionId!==_m.id){await _ack(!1,"That walk is no longer active");return}if(_command==="end"&&awfin.current){await _ack(!0);return}if(!is.current){await _ack(!1,"That walk is no longer active");return}if(_command==="pause"){let _ok=prz.current||await AwToggle();await _ack(_ok,_ok?null:"Could not pause",_ok?awWatchSnapshot():null);return}if(_command==="resume"){let _ok=!prz.current||await AwToggle();await _ack(_ok,_ok?null:"Could not resume",_ok?awWatchSnapshot():null);return}if(_command==="end"){await Bg(!0);let _ok=!is.current;await _ack(_ok,_ok?null:"Could not end the walk");_ok&&await awWatchClear();return}if(_command==="checkIn"){let _spotId=_ev.spotId,_pool=(awroute.current||[]).concat(Z||[],J||[]),_spot=_pool.find(function(_x){return _x&&_x.id===_spotId});if(!_spot){await _ack(!1,"Place is no longer available");return}if(!Se||!Ia){await _ack(!1,"Phone location is not ready");return}let _distance=re(Ia,_spot)*1e3,_radius=Math.max(hw,(Ki||0)+40);if(_distance>_radius){await _ack(!1,"Move closer to check in");return}let _lastCheck=Ct[_spot.id],_age=_lastCheck?Date.now()-Date.parse(_lastCheck):Infinity;if(_age>=0&&_age<18e5){await _ack(!1,"Already checked in recently");return}Ag(_spot);await _ack(!0);return}await _ack(!1,"Unknown command")}catch(_e){await _ack(!1,_e&&_e.message||"Command failed")}},awPlugin=()=>{try{`
);
...
replaceOnce(
  "watch-aware active clear",
  'awClear=()=>{try{',
  'awClear=_deferWatchClear=>{try{'
);

replaceOnce(
  "clear paired Watch session",
  'awroute.current=null,awelapsed.current=null,localStorage.removeItem(AWK),Ae.set(AWK,"").catch(()=>{})}catch(_){}},awReadWalks=',
  'awroute.current=null,awelapsed.current=null,localStorage.removeItem(AWK),Ae.set(AWK,"").catch(()=>{})}catch(_){};_deferWatchClear||awWatchClear()},awReadWalks='
);
...
replaceOnce(
  ...
  'Bg=async _fromWatch=>{awSave(!0),awWatchPublish(!0,"ending");let _np=awnative.current&&awPlugin(),_sid=awid.current&&awid.current.id;_np&&(awfin.current=!0);if(_np)try{let _ns=await _np.stop({sessionId:_sid});_ns&&awApply(_ns)}catch(_e){awfin.current=!1,Do("Couldn’t stop the background recorder — your walk is still safe"),setTimeout(function(){Do("")},4200);return}is.current=!1,prz.current=!1,setPz(!1),setCfe(!1),Sc(!1);let i=ss.current.slice();'
);

replaceOnce(
  "walk end distance source",
  'if(i.length<2)return;let c=pw(i),g=',
  'if(i.length<2){if(_np)try{await _np.discard({sessionId:_sid})}catch(_){}awClear(_fromWatch===!0);return}let c=Math.max(0,+ds.current||0),g='
);
