...
- There is now a **source + build split** (added so the catalogue is editable as
  ...
  - `data/spots.json` — the **9,805 spots** as pretty JSON. **Source of truth for `Z`.**
...
- `npm run build` **refuses to write `index.html`** (writes nothing) if: spots.json is
  ...
  baseline (9,805; see `BASELINE` in `build.js`), if two spots share a name within a
...
- **Reality check (post-scale-up):** with the catalogue at 9,805 spots, the **majority
