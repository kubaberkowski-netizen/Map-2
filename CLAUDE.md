...
- There is a **source + build split** (so the catalogue is editable as clean JSON
  ...
  - `data/spots.json` — the **14,355 spots** as pretty JSON. **Source of truth for `Z`.**
...
- `npm run build` **refuses to write `index.html`** (writes nothing) if: spots.json is
  ...
  baseline (14,355; see `BASELINE` in `build.js`), if two spots share a name within a
...
- `city` **MUST** be a slug defined in the **`Ci` cities registry**. `build.js`
  rejects unknown city slugs. There are **714 cities** today, spanning the UK, Europe,
...
- **Reality check (post-scale-up):** with the catalogue at 14,355 spots, the **majority
...
2. Confirm counts via grep:
   - **entries** — `id:"…",n:"` → should be **14,355** — now counted in the **sidecar**:
