...
- There is a **source + build split** (so the catalogue is editable as clean JSON
  ...
  - `data/spots.json` — the **13,567 spots** as pretty JSON. **Source of truth for `Z`.**
...
- `npm run build` **refuses to write `index.html`** (writes nothing) if: spots.json is
  ...
  baseline (13,567; see `BASELINE` in `build.js`), if two spots share a name within a
...
- **Reality check (post-scale-up):** with the catalogue at 13,567 spots, the **majority
...
2. Confirm counts via grep:
   - **entries** — `id:"…",n:"` → should be **13,567** — now counted in the **sidecar**:
