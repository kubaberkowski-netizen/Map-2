...
- There is a **source + build split** (so the catalogue is editable as clean JSON
  ...
  - `data/spots.json` — the **16,387 spots** as pretty JSON. **Source of truth for `Z`.**
...
- `npm run build` **refuses to write `index.html`** (writes nothing) if: spots.json is
  ...
  baseline (16,387; see `BASELINE` in `build.js`), if two spots share a name within a
...
- `c` **MUST** be one of exactly **45 valid category slugs**. The code reads
...
- Categories are defined in `ne = {slug:{l, e, t}, ...}` (45 slugs;
...
- **Reality check (post-scale-up):** with the catalogue at 16,387 spots, the **majority
...
- `ne` (45 categories) and `Xr` (80 Worlds, which contain live `match: e=>…`
...
2. Confirm counts via grep:
   - **entries** — `id:"…",n:"` → should be **16,387** — now counted across BOTH
   ...
   - **categories** — `(\w+):\{l:"` inside the `ne={…}` block → should be **45**, in the shell:
