...
- The deployed app is **THREE files** (plus an optional fourth: **`fixtures.<hash>.js`** — season sports fixtures matched to stadium spots via `tools/harvest-fixtures.js` → `data/fixtures.json`; loaded async by the same loader as spots.rest, sets `window.__FLFX`, cached in the SW DATA cache; re-run the tool + rebuild weekly/monthly to refresh). Core three (as of the core/rest split): `index.html` — the
  ...
  **`spots.core.<hash>.js`** (~0.6 MB raw — London, the boot city; defines
  `window.__FLZ` and is loaded by a classic `<script src>` **before** the main bundle,
  so boot stays fully synchronous — `window.__FLZ` is set before the app reads
  `Z=window.__FLZ||[]`) and **`spots.rest.<hash>.js`** (~6 MB raw — every other city;
  loaded **async** by a tiny inline loader on `window.load`/first interaction, it
  `push`es into the same `window.__FLZ` array, sets `window.__FLZR=1` and dispatches a
  `flzmore` event; the app bumps a `zv` state on that event so the `Zc`/`fa`/`_a` memos
  re-derive). Cold start therefore ships ~1/10th of the catalogue. **Why split:** the
  ...
  **Settings → Pages** before pushing if in doubt — do not guess. (There is exactly one core+rest **pair** at a time — `build.js` deletes stale `spots.*.js` hashes before writing the new pair, and `sw.js` prunes per kind so the two halves coexist in the DATA cache.)
...
- There is a **source + build split** (so the catalogue is editable as clean JSON
  ...
  - `data/spots.json` — the **15,067 spots** as pretty JSON. **Source of truth for `Z`.**
...
- `npm run build` **refuses to write `index.html`** (writes nothing) if: spots.json is
  ...
  baseline (15,067; see `BASELINE` in `build.js`), if two spots share a name within a
...
- Entries live in an array `Z = [{id, n, a, pc, lat, lng, c, s, q, w, city}, ...]`
  ...
  - Three **additive** fields are injected by `build.js` (NOT stored in `spots.json`):
    `oh` (opening hours, from `data/hours.json`), **`ph`** (a free Wikipedia/Commons
    thumbnail URL, from `data/photos.json` via `tools/enrich-photos.js` — `flThumb`
    uses it as an instant fast path before its runtime geosearch), and **`wq`** (writeup provenance,
...
- `c` **MUST** be one of exactly **52 valid category slugs**. The code reads
...
- Categories are defined in `ne = {slug:{l, e, t}, ...}` (52 slugs;
...
- **Reality check (post-scale-up):** with the catalogue at 15,067 spots, the **majority
...
- `ne` (52 categories) and `Xr` (80 Worlds, which contain live `match: e=>…`
...
2. Confirm counts via grep:
   - **entries** — `id:"…",n:"` → should be **15,067** — now counted across BOTH
   ...
   - **categories** — `(\w+):\{l:"` inside the `ne={…}` block → should be **52**, in the shell:
...
3. Confirm the shell references the core sidecar exactly once and the rest loader once:
   ...
   and `grep -oc 'spots\.rest\.' index.html` (×1, inside the inline async loader).
