...
- There is now a **source + build split** (added so the catalogue is editable as
  ...
  - `data/spots.json` — the **15,346 spots** as pretty JSON. **Source of truth for `Z`.**
...
- `npm run build` **refuses to write `index.html`** (writes nothing) if: spots.json is
  ...
  baseline (15,346; see `BASELINE` in `build.js`), if two spots share a name within a
...
- **Reality check (post-scale-up):** with the catalogue at 15,346 spots, the **majority
...
2. Confirm counts via grep on the HTML:
   - **entries** — `id:"…",n:"` → should be **15,346** (keep in sync with `build.js`'s `BASELINE`)
